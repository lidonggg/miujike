...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='wxGroup'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
	...
	return this.each( function() {
		...
		*/
		...
		this.reDraw = function() {
			$("table.wxGroup:first",this).remove();
		 ...
		 * @param wxGroup - object
		 * @param parentgroup - object
		 */
		this.createTableForGroup = function(group, parentgroup) {
			var that = this,  i;
			// this table will hold all the wxGroup (tables) and rules (rows)
			var table = $("<table class='wxGroup " + classes.table_widget +" ui-search-table' style='border:0px none;'><tbody></tbody></table>"),
			...
			var tr = $("<tr></tr>");
			table.append(tr);
			// this header will hold the wxGroup operator type and wxGroup action buttons for
			// creating subgroup "+ {}", creating rule "+" or deleting the wxGroup "-"
			var th = $("<th colspan='5' align='"+align+"'></th>");
			tr.append(th);

			if(this.p.ruleButtons === true) {
			// dropdown for: choosing wxGroup operator type
			var groupOpSelect = $("<select class='opsel " + classes.srSelect + "'></select>");
			th.append(groupOpSelect);
			// populate dropdown with all posible wxGroup operators: or, and
			var str= "", selected;
			...
			if(this.p.groupButton) {
				inputAddSubgroup = $("<input type='button' value='+ {}' title='Add subgroup' class='add-wxGroup " + common.button + "'/>");
			...
			// button for delete the wxGroup
			if (parentgroup !== null) { // ignore the first wxGroup
				var inputDeleteGroup = $("<input type='button' value='-' title='Delete wxGroup' class='delete-wxGroup " + common.button + "'/>");
				th.append(inputDeleteGroup);
				inputDeleteGroup.bind('click',function() {
				// remove wxGroup from parent
				...
				if (p.searchOnEnter || p.closeOnEscape) {
					$("#"+$.jgrid.jqID(IDs.themodal)).keydown(function (e) {
						...
						if (p.searchOnEnter && e.which === 13 && // 13 === $.ui.keyCode.ENTER
								!$target.hasClass('add-wxGroup') && !$target.hasClass('add-rule') &&
								!$target.hasClass('delete-wxGroup') && !$target.hasClass('delete-rule') &&
...
$.jgrid.extend({
	...
	groupingPrepare : function ( record, irow ) {
		this.each(function(){
			...
			for(i=0;i<grlen;i++) {
				...
				if( v !== undefined ) {
					if(irow === 0 ) {
						// First record always starts a new wxGroup
						...
						if (typeof v !== "object" && ($.isArray(grp.isInTheSameGroup) && $.isFunction(grp.isInTheSameGroup[i]) ? ! grp.isInTheSameGroup[i].call($t, grp.lastvalues[i], v, i, grp): grp.lastvalues[i] !== v)) {
							// This record is not in same wxGroup as previous one
							grp.groups.push({idx:i,dataIndex:fieldName,value:v, displayValue: displayValue, startRow: irow, cnt:1, summary : [] } );
							grp.lastvalues[i] = v;
							changed = 1;
							grp.counters[i] = {cnt:1, pos:grp.groups.length-1, summary: $.extend(true,[],grp.summary)};
							...
							if (changed === 1) {
								// This wxGroup has changed because an earlier wxGroup changed.
								grp.groups.push({idx:i,dataIndex:fieldName,value:v, displayValue: displayValue, startRow: irow, cnt:1, summary : [] } );
								grp.lastvalues[i] = v;
								grp.counters[i] = {cnt:1, pos:grp.groups.length-1, summary: $.extend(true,[],grp.summary)};
								$.each(grp.counters[i].summary, sumGroups);
				...
				if(r){
					...
					while(r) {
						...
						if (showData === undefined) {
							showData = itemGroupingLevel === undefined; // if the first row after the opening wxGroup is data row then show the data rows
						}
	...
	groupingGroupBy : function (name, options ) {
		return this.each(function(){
			...
			// set visibility status of current wxGroup columns on next grouping
...
$.jgrid.extend({
	pivotSetup : function( data, options ){
		...
		// again with wxGroup setup options which then will be passed to the grid
		var columns =[],
		pivotrows =[],
		summaries = [],
		member=[],
		...
		this.each(function(){
			 ...
			 * columns from the pivot values and set the wxGroup Headers
			 */
			function list(items) {
				var l, j, key, k, col;

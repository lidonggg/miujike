...
layui.define(['element', 'common'], function (exports) {
    ...
    function setOnClick(elem, callback) {
        if (elem.find('dl').length > 0) {
            elem.find('dd').each(function () {
                var $dd = $(this);
                if ($dd.children('a').data('url') !== undefined) {
    ...
    function getHtml(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                ulHtml = getDlHtml(ulHtml, data, i);
            } else {
                ulHtml = getNoChildHtml(ulHtml, data, i);
            }
            ulHtml += '</li>';
        }
        ulHtml += '</ul>';
    ...
    function getDlHtml(ulHtml, data, i) {
        ...
        for (var j = 0; j < data[i].children.length; j++) {
            ...
            if (data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0) {
                ulHtml = getDlHtml(ulHtml, data[i].children, j);
            } else {
                ulHtml = getNoChildHtml(ulHtml, data[i].children, j);
            }
            ulHtml += '</dd>';
        }
        ulHtml += '</dl>';
    ...
    function getNoChildHtml(ulHtml, data, i) {
        var dataUrl = (data[i].href !== undefined && data[i].href !== '') ? 'data-url="' + data[i].href + '"' : '';
        ulHtml += '<a href="javascript:;" ' + dataUrl + '>';
        if (data[i].icon !== undefined && data[i].icon !== '') {
            if (data[i].icon.indexOf('fa-') !== -1) {
